# alpha_not_scaled.py - Testing JAGS fits of a non-hierarchical DDM model with external data predicted by boundary in Python 3
#
...
if not os.path.exists('data/alpha_not_scaled_test1.mat'):
    # Number of simulated participants
    nparts = 100 # tests 1 and 2
    # nparts = 500 # test 3 and 4
    ...
    # sigma = .5 # Test 4
    # sigma = .01  # Test 2 & 3, Very low measurement noise compared to standard deviation of boundaries
    sigma = .1 # Test 1, low measurement noise compared to standard deviation of boundaries
    y = np.zeros(N)
    rt = np.zeros(N)
    acc = np.zeros(N)
    ...
    genparam['N'] = N
    sio.savemat('data/alpha_not_scaled_test1.mat', genparam)
else:
    genparam = sio.loadmat('data/alpha_not_scaled_test1.mat')
...
modelstring = 'modelfits/alpha_not_scaled_test1.mat'
...
modelfile = 'jagscode/alpha_not_scaled_test1.jags'
f = open(modelfile, 'w')
f.write(tojags)
f.close()
...
plot_path = f"new_figures/alpha_not_scaled_test1"
...
plt.savefig(f'{plot_path}/sigma_posterior.png', format='png', bbox_inches="tight")
plt.close()
