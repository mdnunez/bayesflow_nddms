...
def simuldiff2ndt(N=100, Alpha=1, Vet=.2, Rmr=.2, Nu=1, Zeta=None, rangeVet=0, rangeRmr=0, rangeZeta=0, Eta=.3,
                  Varsigma=1):
    """
    SIMULDIFF2NDT  Generates data according to a diffusion model with non-decision time split into two parts with
    ...
    Returns
    -------
    Numpy complex vector with 1) Real component ( np.real(x) ): reaction times (in seconds) multiplied by the response
    vector such that negative reaction times encode response B and positive reaction times
...
def logwienerpdf(y, Alpha=1, Tau=.4, Nu=1, Beta=.5, Err=10e-29):
    """
    LOGWIENERPDF  log of the Wiener first-pass time pdf with y=response*rt input
    ...
    Parameters
    ----------
    y:  Numpy array with reaction times (in seconds) multiplied by the response vector
    such that negative reaction times encode response B and positive reaction times
    encode response A 

    Alpha: the boundary separation across trials in evidence units
    (defaults to 1 evidence unit)

    Tau: the non-decision time across trials in seconds
    (defaults to .4 seconds)

    Nu: the drift rate across trials in evidence units per second
    (defaults to 1 evidence units per second)

    Beta: the initial bias in the evidence process for choice A as a proportion of boundary Alpha
    Note that this is the relative start point and not the absolute start point
    (defaults to .5 or 50% of total evidence units given by Alpha)
    ...
    See also http://ppw.kuleuven.be/okp/dmatoolbox.
    """
    time = np.abs(y)  # Time t
    choice = np.array((y > 0), dtype=int)  # 1 and 0 indicator for choices
    N = y.shape[0]  # Number of observations
    if any(t < Tau):
        logdensity = -np.ninf  # density = 0, thus log(density) = -Inf
        # logdensity = -sys.float_info.max
    else:
        normtime = (time - Tau) / (np.power(Alpha, 2))  # Normalize time by boundary separation
        logdensity = 0
        for m in range(0, N):
            if (choice(m) == 1):
                drift = Nu
            ...
            if np.pi * normtime * Err < 1:  # If the error threshold is set low enough
                kl = np.sqrt()  ###UNFINISHED###
