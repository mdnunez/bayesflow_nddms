...
# ====         ================                       ======================
# 26-June-23     Michael Nunez     Combination of ideas from single_trial_alpha_not_scaled
#                                    and single_trial_alpha_dc
...
def draw_prior():
    ...
    # std_dc ~ N(1.0, 0.5) in [0, 3], trial-to-trial std in diffusion coefficient, index 6
    std_dc = truncnorm_better(mean=1.0, sd=0.5, low=0.0, upp=3)[0]

    # gamma_bd1 = 1 Fixed effect of single-trial boundary on extdata1

    # gamma_dc1 ~ N(0, 1.0), Effect of single-trial diffusion coefficient on extdata1, index 7
    ...
    # sigma1 ~ U(0.0, 5.0),measurement noise of extdata1, index 9
    sigma1 = RNG.uniform(0.0, 5.0)

    # sigma2 ~ U(0.0, 5.0),measurement noise of extdata2, index 10
    sigma2 = RNG.uniform(0.0, 5.0)

    p_samples = np.hstack((drift, mu_alpha, beta, ter, std_alpha, mu_dc, std_dc, gamma_dc1,
...
@njit
def diffusion_trial(drift, mu_alpha, beta, tau, std_alpha, mu_dc, std_dc, gamma_dc1,
    ...
    extdata1 = np.random.normal(1*bound_trial + gamma_dc1*dc_trial, sigma1)
    extdata2 = np.random.normal(gamma_bd2*bound_trial + 1*dc_trial, sigma2)
    ...
    return choicert, extdata1, extdata2

@njit
def simulate_trials(params, n_trials):
    """Simulates a diffusion process for trials ."""

    drift, mu_alpha, beta, tau, std_alpha, mu_dc, std_dc, gamma_dc1, gamma_bd2, sigma1, sigma2 = params
    choicert = np.empty(n_trials)
    z1 = np.empty(n_trials)
    z2 = np.empty(n_trials)
    for i in range(n_trials):
        choicert[i], z1[i], z2[i] = diffusion_trial(drift, mu_alpha, beta, tau, std_alpha, mu_dc, 
            std_dc, gamma_dc1, gamma_bd2, sigma1, sigma2)
...
if view_simulation:
    ...
    extdata2_means =np.empty((num_test))
    extdata2_stds = np.empty((num_test))
    ...
    for i in range(num_test):
        ...
        extdata1_stds[i] = np.std(np.squeeze(these_sims[0, :, 1]))
        extdata2_means[i] = np.mean(np.squeeze(these_sims[0, :, 2]))
        extdata2_stds[i] = np.std(np.squeeze(these_sims[0, :, 2]))
    ...
    plt.figure()
    sns.kdeplot(extdata2_means)
    plt.xlabel('External data2 means')

    plt.figure()
    sns.kdeplot(extdata2_stds)
    plt.xlabel('External data2 standard deviations')
    ...
    plt.xlabel('External data2 values')
...
fig = bf.diagnostics.plot_recovery(param_samples[0:500,:], true_params[0:500,:], param_names =
    ['mu_drift', 'mu_alpha', 'beta', 'tau', 'std_alpha', 'dc', 'std_dc', 'gamma_bd1',
...
publication_text = rf"""
Draws from a joint posterior distribution for one simulated data set from Model dcDDM-$\alpha \varsigma z$ 
...
# trial-to-trial standard deviation in boundary - index 4
std_alpha = 1

# mean diffusion coefficient - index 5
mu_dc = 1

# trial-to-trial standard deviation in diffusion coefficient - index 6
std_dc = 1
